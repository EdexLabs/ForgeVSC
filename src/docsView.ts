...
import {
    GuideMetadata, getGuides, reloadGuides,
    isFavorite, addFavorite, removeFavorite,
    buildGuideUrl, guideTitle, guideCategory, DOCS_BASE,
} from './guides';
...
export interface EventField { name: string; description?: string; }
export interface EventMeta { name: string; description?: string; fields?: EventField[]; extension?: string; }
export interface EnumMeta { name: string; values: string[]; extension?: string; }
...
let memFns: FnMeta[] | null = null;
let memEnums: EnumMeta[] | null = null;
let memEvents: EventMeta[] | null = null;

let inFns: Promise<FnMeta[]> | null = null;
let inEnums: Promise<EnumMeta[]> | null = null;
let inEvents: Promise<EventMeta[]> | null = null;
    ...
    return new Promise((resolve, reject) => {
        const go = (u: string) => {
            https.get(u, { headers: { 'User-Agent': 'forgescript-docs/2.0' } }, res => {
                if ((res.statusCode === 301 || res.statusCode === 302) && res.headers.location) { go(res.headers.location); return; }
                if (res.statusCode !== 200) { reject(new Error(`HTTP ${res.statusCode}`)); return; }
                const b: Buffer[] = [];
                res.on('data', (c: Buffer) => b.push(c));
                res.on('end', () => { try { resolve(JSON.parse(Buffer.concat(b).toString('utf8'))); } catch (e) { reject(e); } });
                res.on('error', reject);
            }).on('error', reject);
        };
        go(url);
...
interface ExtEntry { id: string; label: string; functions?: string; enums?: string; events?: string; }

function resolveExtensions(): ExtEntry[] {
...
export async function getFunctions(force = false): Promise<FnMeta[]> {
    if (memFns && !force) return memFns;
    if (inFns) return inFns;
    inFns = (async () => {
        if (!force) {
            const s = ctx.globalState.get<FnMeta[]>(CK_FNS);
            if (s?.length) { memFns = s; return s; }
        }
        const all: FnMeta[] = [];
        for (const ext of resolveExtensions()) {
            if (!ext.functions) continue;
            try {
                const d = await getJson(ext.functions) as FnMeta[];
                all.push(...d.map(f => ({ ...f, extension: f.extension ?? ext.id })));
            } catch (e) { log?.appendLine(`[Docs] fn fetch failed ${ext.id}: ${e}`); }
        }
        memFns = all;
...
export async function getEnums(force = false): Promise<EnumMeta[]> {
    if (memEnums && !force) return memEnums;
    if (inEnums) return inEnums;
    inEnums = (async () => {
        if (!force) {
            const s = ctx.globalState.get<EnumMeta[]>(CK_ENUMS);
            if (s?.length) { memEnums = s; return s; }
        }
        const all: EnumMeta[] = [];
        for (const ext of resolveExtensions()) {
            if (!ext.enums) continue;
            try {
                const d = await getJson(ext.enums) as Record<string, string[]>;
                all.push(...Object.entries(d).map(([name, values]) => ({ name, values, extension: ext.id })));
            } catch (e) { log?.appendLine(`[Docs] enum fetch failed ${ext.id}: ${e}`); }
        }
        memEnums = all;
...
export async function getEvents(force = false): Promise<EventMeta[]> {
    if (memEvents && !force) return memEvents;
    if (inEvents) return inEvents;
    inEvents = (async () => {
        if (!force) {
            const s = ctx.globalState.get<EventMeta[]>(CK_EVENTS);
            if (s?.length) { memEvents = s; return s; }
        }
        const all: EventMeta[] = [];
        for (const ext of resolveExtensions()) {
            if (!ext.events) continue;
            try {
                const d = await getJson(ext.events) as EventMeta[];
                all.push(...d.map(e => ({ ...e, extension: e.extension ?? ext.id })));
            } catch (e) { log?.appendLine(`[Docs] event fetch failed ${ext.id}: ${e}`); }
        }
        memEvents = all;
...
async function reloadAll(force = true): Promise<void> {
    ...
    await Promise.all([getFunctions(force), getEnums(force), getEvents(force), reloadGuides()]);
}

// ─── Display helpers ───────────────────────────────────────────────────────

function extLabel(id?: string): string {
    if (!id) return 'ForgeScript';
...
function argTypeFmt(a: FnArg): string {
    if (Array.isArray(a.type)) return a.type.join(' | ');
    return (a.type as string | undefined) ?? 'any';
}
...
function guideMatchesExt(g: GuideMetadata, extId: string): boolean {
...
type NodeKind =
    | 'extension'
    | 'fn-section' | 'fn-category' | 'function'
    | 'enum-section' | 'enum'
    | 'event-section' | 'event'
    | 'guide-section' | 'guide-favorites' | 'guide-category' | 'guide-subcategory' | 'guide';
...
class ExplorerProvider implements vscode.TreeDataProvider<DocNode> {
    private _fire = new vscode.EventEmitter<DocNode | void>();
    readonly onDidChangeTreeData = this._fire.event;
    refresh(): void { this._fire.fire(); }

    getTreeItem(n: DocNode): vscode.TreeItem {
        // ── Extension root ──
        if (n.kind === 'extension') {
        ...
        if (n.kind === 'guide' && n.guide) {
            ...
            item.id = `guide::${n.uid}`;
            item.iconPath = new vscode.ThemeIcon('book');
            item.contextValue = isFavorite(g.id) ? 'guide.favorite' : 'guide';
    ...
    async getChildren(parent?: DocNode): Promise<DocNode[]> {
        ...
        if (!parent) {
            ...
            const guideOnlyExts = [...new Set(guides.map(g => g.packageName ?? ''))].filter(pkg =>
                !extIds.some(id => guideMatchesExt({ packageName: pkg } as GuideMetadata, id))
            );

            return [
                ...extIds.map<DocNode>(id => ({ kind: 'extension', uid: id, label: extLabel(id), extId: id })),
                ...guideOnlyExts.map<DocNode>(pkg => ({ kind: 'extension', uid: `guide-only::${pkg}`, label: pkg, extId: `guide-only::${pkg}` })),
            ];
        }
        ...
        if (parent.kind === 'guide-section') {
            ...
            const cats = [...new Set(inExt.map(g => guideCategory(g)))].sort();
            const nodes: DocNode[] = [];

            const hasFavs = inExt.some(g => isFavorite(g.id));
            if (hasFavs)
                nodes.push({ kind: 'guide-favorites', uid: `${parent.extId}::guide-favs`, label: 'Favorites', extId: parent.extId });

            nodes.push(...cats.map<DocNode>(cat => ({
        ...
        if (parent.kind === 'guide-favorites') {
            const guides = await getGuides();
            return guides
                .filter(g => guideMatchesExt(g, parent.extId!) && isFavorite(g.id))
                .sort((a, b) => guideTitle(a).localeCompare(guideTitle(b)))
                .map<DocNode>(g => ({
                    kind: 'guide', uid: `fav::${g.id}`, label: guideTitle(g), extId: parent.extId, guide: g,
                }));
        }
        ...
        if (parent.kind === 'guide-category') {
            const guides = await getGuides();
            const inCat = guides.filter(g => guideMatchesExt(g, parent.extId!) && guideCategory(g) === parent.category);
            const subs = [...new Set(inCat.map(g => g.subCategory?.trim()).filter(Boolean) as string[])].sort();

            const subNodes = subs.map<DocNode>(sub => ({
            ...
            const direct = inCat
                ...
                .sort((a, b) => guideTitle(a).localeCompare(guideTitle(b)))
                .map<DocNode>(g => ({
                    kind: 'guide', uid: `guide::${g.id}`, label: guideTitle(g),
        ...
        if (parent.kind === 'guide-subcategory') {
            const guides = await getGuides();
            return guides
                .filter(g => guideMatchesExt(g, parent.extId!) && guideCategory(g) === parent.category && g.subCategory?.trim() === parent.subCategory)
                .sort((a, b) => guideTitle(a).localeCompare(guideTitle(b)))
                .map<DocNode>(g => ({
                    kind: 'guide', uid: `guide::${g.id}`, label: guideTitle(g),
...
function showPanel(title: string, html: string): void {
    if (activePanel) {
        ...
        activePanel = vscode.window.createWebviewPanel(
            'forgescript.docs', title, { viewColumn: vscode.ViewColumn.One, preserveFocus: true },
...
type SearchItem = vscode.QuickPickItem & { docKind: 'fn' | 'enum' | 'event' | 'guide'; payload: FnMeta | EnumMeta | EventMeta | GuideMetadata; };
...
export async function runSearch(): Promise<void> {
    ...
    const allItems: SearchItem[] = [
        ...
        ...guides.map<SearchItem>(g => ({
            label: `$(book) ${guideTitle(g)}`,
            description: g.packageName,
            detail: guideCategory(g) + (g.subCategory ? ` › ${g.subCategory}` : ''),
            docKind: 'guide',
            payload: g,
        })),
    ];
    ...
    qp.onDidChangeValue(value => {
        let filter: 'fn' | 'enum' | 'event' | 'guide' | null = null;
        let query = value;

        if (value.startsWith('$')) { filter = 'fn'; query = value.slice(1); }
        else if (value.startsWith('/')) { filter = 'guide'; query = value.slice(1); }
        else if (value.startsWith('?')) { filter = 'enum'; query = value.slice(1); }
        else if (value.startsWith('.')) { filter = 'event'; query = value.slice(1); }
    ...
    qp.onDidAccept(() => {
        ...
        if (!sel) return;
        qp.hide();
        if (sel.docKind === 'fn') { const p = sel.payload as FnMeta; showPanel(p.name, buildFnHtml(p)); }
        if (sel.docKind === 'enum') { const p = sel.payload as EnumMeta; showPanel(p.name, buildEnumHtml(p)); }
        if (sel.docKind === 'event') { const p = sel.payload as EventMeta; showPanel(p.name, buildEventHtml(p)); }
        if (sel.docKind === 'guide') { const p = sel.payload as GuideMetadata; showPanel(guideTitle(p), buildGuideHtml(p)); }
    });
...
function buildFnHtml(fn: FnMeta): string {
    const args = fn.args ?? [];
    ...
    const links = [
        `<a class="link-btn" href="${DOCS_BASE}function/$${esc(slug)}?p=${esc(pkgId)}">📖 Docs</a>`,
...
function buildEnumHtml(e: EnumMeta): string {
...
function buildEventHtml(e: EventMeta): string {
    const rows = (e.fields ?? []).map(f =>
        `<tr><td><code class="t-name">${esc(f.name)}</code></td><td>${esc(f.description ?? '')}</td></tr>`
    ).join('');
    return shell(e.name, `
<div class="hdr">
  <div class="hdr-icon">🔔</div>
...
function mdToHtml(md: string): string {
    ...
    for (let raw of lines) {
        ...
        // Headings
        if (raw.startsWith('### ')) { out.push(`<h3>${inlineM(raw.slice(4))}</h3>`); continue; }
        if (raw.startsWith('## ')) { out.push(`<h2>${inlineM(raw.slice(3))}</h2>`); continue; }
        if (raw.startsWith('# ')) { out.push(`<h1>${inlineM(raw.slice(2))}</h1>`); continue; }

        // Blockquotes
        if (raw.startsWith('> ')) { out.push(`<blockquote><p>${inlineM(raw.slice(2))}</p></blockquote>`); continue; }
...
function inlineM(s: string): string {
    return esc(s)
        // bold
...
function buildGuideHtml(g: GuideMetadata): string {
...
export function registerDocsView(extCtx: vscode.ExtensionContext, channel: vscode.OutputChannel): void {
    ctx = extCtx;
    log = channel;
    ...
    extCtx.subscriptions.push(
        vscode.window.createTreeView('forgescript.explorerView', {
    ...
    extCtx.subscriptions.push(
        // ── Docs openers ──
        vscode.commands.registerCommand('forgescript.openFunctionDocs', (input: FnMeta | string) => {
            if (typeof input === 'string') { getFunctions().then(fns => { const f = fns.find(f => f.name.toLowerCase() === input.toLowerCase()); f ? showPanel(f.name, buildFnHtml(f)) : vscode.window.showErrorMessage(`Function "${input}" not found.`); }); }
            else { showPanel(input.name, buildFnHtml(input)); }
        }),

        vscode.commands.registerCommand('forgescript.openEnumDocs', (input: EnumMeta | string) => {
            if (typeof input === 'string') { getEnums().then(es => { const e = es.find(e => e.name.toLowerCase() === input.toLowerCase()); e ? showPanel(e.name, buildEnumHtml(e)) : vscode.window.showErrorMessage(`Enum "${input}" not found.`); }); }
            else { showPanel(input.name, buildEnumHtml(input)); }
        }),

        vscode.commands.registerCommand('forgescript.openEventDocs', (input: EventMeta | string) => {
            if (typeof input === 'string') { getEvents().then(es => { const e = es.find(e => e.name.toLowerCase() === input.toLowerCase()); e ? showPanel(e.name, buildEventHtml(e)) : vscode.window.showErrorMessage(`Event "${input}" not found.`); }); }
            else { showPanel(input.name, buildEventHtml(input)); }
        }),

        // Open guide in webview panel
        vscode.commands.registerCommand('forgescript.openGuide', (input: GuideMetadata | number | string) => {
            const open = (g: GuideMetadata) => showPanel(guideTitle(g), buildGuideHtml(g));
            if (typeof input === 'object' && input) { open(input as GuideMetadata); return; }
            getGuides().then(all => {
                const id = typeof input === 'number' ? input : Number(input);
                const g = !isNaN(id) ? all.find(g => g.id === id) : all.find(g => guideTitle(g).toLowerCase() === String(input).toLowerCase());
    ...
    // Background preload
    Promise.all([getFunctions(), getEnums(), getEvents(), getGuides()]).then(() => provider.refresh());

    // Invalidate on forgeconfig changes
    const w = vscode.workspace.createFileSystemWatcher('**/forgeconfig.json');
    extCtx.subscriptions.push(w,
        w.onDidChange(async () => { await reloadAll(); provider.refresh(); }),
        w.onDidCreate(async () => { await reloadAll(); provider.refresh(); }),
    );
}
